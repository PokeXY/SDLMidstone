...
Scene3::~Scene3() {// Rember to delete every pointer NO MEMORY LEAKS!!!!!!

	OnDestroy();
}

bool Scene3::OnCreate(float GOF) {
	int w, h;
	float xAxis = 32.0f;
	float yAxis = 18.0f;
	float zAxis = 1.0f;
...
void Scene3::HandleEvents(const SDL_Event& sdlEvent) { //Make stuff happen here with the clickety clack
