...
Scene4::~Scene4() {// Rember to delete every pointer NO MEMORY LEAKS!!!!!!

	OnDestroy();
}

bool Scene4::OnCreate(float GOF) {
	int w, h;
	float xAxis = 32.0f;
	float yAxis = 18.0f;
	float zAxis = 1.0f;
	...
	player->setPos(Vec3(3.0f, 7.0f, 0.0f));
	...
	for (int i = 0; i < 3; ++i) {
		...
		enemies[i]->setPos(Vec3(xAxis - 3.0f, yAxis - 7.0f - 2.0f * i, 0.0f));
	...
	healthPickup->setPos(Vec3(28.0f, 9.0f, 0.0f));
...
void Scene4::HandleEvents(const SDL_Event& sdlEvent) { //Make stuff happen here with the clickety clack
